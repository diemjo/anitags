...
int main(int argc, char* argv[]) {
    ...
    if (conf->tags_to_remove.size() > 0) {
        db_remove_tags(conf);
        if (conf->modifyExif)
            exif_remove_tags(conf, et);
    }

    if (conf->tags_to_add.size() > 0) {
        db_add_tags(conf);
        if (conf->modifyExif)
            exif_add_tags(conf, et);
    }

    if (conf->modifyTags) {
        if (conf->modifyExif)
            exif_write_tags(conf, et, argv[0]);
    }
    ...
    else if (conf->importTags) {
        import_tags(conf, et,argv[0]);
    }
    ...
    //Really slow, not really necessary
    delete et;
    free(conf);
...
void print_tags(config *conf, ExifTool *et, char* argv0) {
    ...
    /*for (string filename : conf->filenames) {
        vector<string> tags = exif_get_tags(filename, et, argv0);
        if (tags.size()>0) {
...
void import_tags(config *conf, ExifTool *et, char* argv0) {
    ...
    for (string file : filenames) {
        vector<string> tags = exif_get_tags(file, et, argv0);
        conf->filenames.clear();
        conf->filenames.push_back(file);
        conf->tags_to_add.clear();
        conf->tags_to_add.insert(conf->tags_to_add.end(), tags.begin(), tags.end());
        db_add_tags(conf);
    }
}
...
void export_tags(config *conf, ExifTool *et, char* argv0) {
    ...
    for (string file : filenames) {
        conf->filenames.clear();
        conf->filenames.push_back(file);
        conf->tags_to_add = db_get_tags(conf)[0].second;
        exif_add_tags(conf, et);
        exif_write_tags(conf, et, argv0);
